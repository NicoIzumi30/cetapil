            ...
            <div class="grid grid-cols-2 items-center my-2">
                ...
                <div class="flex items-center gap-4">
                    <div>
                        ...
                        <input id="stock-500" class="form-control" type="number" name="stock-500"
                            placeholder="Masukan Jumlah Stock" aria-describedby="stock-500" />
                    </div>
                    <div>
                        <label for="selling-500" class="form-label">Selling</label>
                        <input id="selling-500" class="form-control" type="number" name="selling-500"
                            placeholder="Masukan Jumlah Selling" aria-describedby="selling-500" />
                    </div>
                    <div>
                        <label for="balance-500" class="form-label">Balance</label>
                        <input id="balance-500" class="form-control" type="number" name="balance-500"
                            placeholder="Masukan Jumlah Balance" aria-describedby="balance-500" />
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 items-center my-2">
                ...
                <div class="flex items-center gap-4">
                    <div>
                        ...
                        <input id="stock-400" class="form-control" type="number" name="stock-400"
                            placeholder="Masukan Jumlah Stock" aria-describedby="stock-400" />
                    </div>
                    <div>
                        <label for="selling-400" class="form-label">Selling</label>
                        <input id="selling-400" class="form-control" type="number" name="selling-400"
                            placeholder="Masukan Jumlah Selling" aria-describedby="selling-400" />
                    </div>
                    <div>
                        <label for="balance-400" class="form-label">Balance</label>
                        <input id="balance-400" class="form-control" type="number" name="balance-400"
                            placeholder="Masukan Jumlah Balance" aria-describedby="balance-400" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 items-center my-6">
            ...
            <div class="flex items-center gap-4">
                <div>
                    ...
                    <input id="stock-200" class="form-control" type="number" name="stock-200"
                        placeholder="Masukan Jumlah Stock" aria-describedby="stock-200" />
                </div>
                <div>
                    <label for="selling-200" class="form-label">Selling</label>
                    <input id="selling-200" class="form-control" type="number" name="selling-200"
                        placeholder="Masukan Jumlah Selling" aria-describedby="selling-200" />
                </div>
                <div>
                    <label for="balance-200" class="form-label">Balance</label>
                    <input id="balance-200" class="form-control" type="number" name="balance-200"
                        placeholder="Masukan Jumlah Balance" aria-describedby="balance-200" />
                </div>
            </div>
        </div>
        ...
        <x-section-card :title="'Tambahkan Foto Outlet'">
            <div class="flex flex-col items-center w-full">
                <div class="relative w-full mx-3 border-red">
                    <div class="flex justify-center items-center flex-col py-2 h-[260px]">
                        ...
                        <div class="cursor-pointer text-center grid place-items-center border-2 border-dashed border-blue-400 rounded-lg p-4 w-full h-full"
                            ...
                            <div class="text-center grid place-items-center">
                                ...
                                <h5 class="text-white font-bold text-xl mt-2">Klik disini untuk mulai mengunggah gambar
                                </h5>
                        ...
                        {{-- Preview Container --}}
                        <div id="preview-container-outlet"
                            class="hidden w-full relative border-2 border-dashed border-blue-400 rounded-lg">
                            <img id="preview-image-outlet" src="/" alt="Preview"
                    ...
                    {{-- Hidden File Input --}}
                    <input type="file" name="outlet" id="img_outlet" class="hidden" accept="image/png, image/jpeg"
                        onchange="previewImage(this, 'outlet', 5)">
                </div>
            </div>
        </x-section-card>
...
@push('scripts')
    <script>
        $(document).ready(function() {
			...
			function getBalance(idStock, idSelling, idBalance) {
				const stock = $(`#${idStock}`).val();
				const selling = $(`#${idSelling}`).val();
				$(`#${idBalance}`).val(sumBalance(stock, selling))
			}

            $('#stock-200, #selling-200').keyup(function() {
				getBalance('stock-200', 'selling-200', 'balance-200')
            });
            $('#stock-400, #selling-400').keyup(function() {
				getBalance('stock-400', 'selling-400', 'balance-400')
            });
            $('#stock-500, #selling-500').keyup(function() {
				getBalance('stock-500', 'selling-500', 'balance-500')
            });
