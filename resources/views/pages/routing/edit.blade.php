            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="code" class="form-control @error('code') is-invalid @enderror"
                        value="{{ $outlet->code }}" type="text" name="code" placeholder="Masukan nama outlet"
                ...
                <div>
                    ...
                    <input id="outlet_type" class="form-control @error('outlet_type') is-invalid @enderror"
                        value="{{ $outlet->tipe_outlet }}" {{-- Change from old() to outlet value --}} type="text" name="outlet_type"
                ...
                <div>
                    ...
                    <input id="account_type" class="form-control @error('account_type') is-invalid @enderror"
                        value="{{ $outlet->account }}" {{-- Change from old() to outlet value --}} type="text" name="account_type"
                ...
                <div id="cycles-wrapper" class="grid gap-4">
                    @foreach ($outlet->outletRoutings as $routing)
                        <div class="flex items-end gap-4" data-cycle-id="{{ $loop->iteration }}">
                            <div class="flex-1">
                                <label for="visit_day_{{ $loop->iteration }}">Waktu Kunjungan</label>
                                <select id="visit_day_{{ $loop->iteration }}" name="visit_day[]" class="w-full">
                            ...
                            <div class="flex-1">
                                <label for="week_{{ $loop->iteration }}" class="form-label">Week</label>
                                <select id="week_{{ $loop->iteration }}" name="week[]" class="w-full">
                            ...
                            @else
                                <button type="button"
                                    class="px-6 py-4 bg-red-400 rounded-md hover:bg-red-500 transition-all"
                                    onclick="removeCycle({{ $loop->iteration }})">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
            ...
            @foreach ($categories as $category)
                <div id="{{ \Str::slug($category->name) }}" class="category-field border-b-2 border-dashed py-6">
                    ...
                    @foreach ($category->products as $product)
                        @php
                            $av3mValue =
                                App\Models\Av3m::where('outlet_id', $outlet->id)
                        ...
                        <div class="grid grid-cols-3 items-center p-4 rounded-lg">
                            ...
                            <div>
                                ...
                                <input type="number" name="av3m[{{ $product->sku }}]" id="av3m-{{ $product->sku }}"
                                    class="form-control av3m-input w-full" min="0"
            ...
            <x-section-card :title="'Area Domisili Outlet'">
                <div class="grid grid-cols-2 gap-6">
                        ...
                        <select id="city" name="city" class=" w-full">
                            ...
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}"
                                    {{ $city->id == ($outlet->city['id'] ?? '') ? 'selected' : '' }}>
                                    {{ $city->name }}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('city'))
                    ...
                    <div>
                        ...
                        <input id="adresss" class="form-control" value="{{ $outlet->address }}" type="text"
                            name="adresss" placeholder="Masukkan Alamat Lengkap" aria-describedby="adresss" />
                            ...
                            <div class="flex justify-between items-center w-full">
                                <div class="flex justify-between items-center w-full">
                                    ...
                                    <div class="relative inline-flex items-center">
                                        <input type="checkbox" name="survey[{{ $form->id }}]" id="gih-checkbox"
                                            checked
                                            value="{{ $form->answers->where('outlet_id', $outlet->id)->first()?->answer ?? 'Sudah' }}"
                                            class="sr-only" />
...
@push('scripts')
    <script>
        // add cycle row 
        let cycleCounter = 1;
		const outletRoutings = @json($outlet->outletRoutings); 
		outletRoutings.forEach((item, index) => {
			cycleCounter++;
			$(`#visit_day_${index + 1}, #week_${index + 1}`).select2()
		})  
        function addCycle() {
            cycleCounter++;
