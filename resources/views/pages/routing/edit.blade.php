...
<form action="{{ route('routing.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input id="name" class="form-control @error('name') is-invalid @enderror" type="text" name="name" value="{{$outlet->name}}" placeholder="Masukan nama outlet"
        ...
        <div>
            ...
            <select id="user_id" name="user_id" class="w-full">
                ...
                @foreach ($salesUsers as $sales)
                    <option value="{{ $sales->id }}" {{$sales->id == $outlet->user_id ? 'selected' : ''}}>{{ $sales->name }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="visit_day" name="visit_day" class=" w-full">
                ...
                @foreach ($waktuKunjungan as $hari)
                    <option value="{{$hari['value']}}" {{$hari['value'] == $outlet->visit_day ? 'selected' : ''}}>{{$hari['name']}}</option>
                @endforeach
            </select>
        ...
        <div id="week-container" class="hidden">
            ...
            <select id="week_type" name="week_type" class=" w-full">
                ...
                @foreach ($weekType as $week)
                    <option value="{{$week['value']}}" {{$week['value'] == $outlet->week_type ? 'selected' : ''}}>{{$week['name']}}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select id="channel" name="channel" class=" w-full">
                ...
                @foreach ($channels as $channel)
                    <option value="{{$channel->id}}" {{$channel->id == $outlet->channel ? 'selected' : ''}}>{{$channel->name}}</option>
                @endforeach
            </select>
           ...
           <select id="product_category" name="product_category[]" class="w-full" multiple="multiple">
               @foreach ($categories as $category)
                   <option value="{{ $category->id }}" 
                       ...
                       {{ $category->products->isNotEmpty() ? 'selected' : '' }}>
                       {{ $category->name }}
                   </option>
               @endforeach
           </select>
    ...
    <x-section-card :title="'Area Domisili Outlet'">
        <div class="grid grid-cols-2 gap-6">
            <div>
                ...
                <input id="longitude" class="form-control @error('longtitude') is-invalid @enderror" value="{{ $outlet->longitude }}" type="text" name="longitude"
            ...
            <div>
                <label for="latitude" class="form-label">Latitudes <span class="font-normal">(DMS
                        Coordinates)</span></label>
                <input id="latitude" class="form-control @error('latitude') is-invalid @enderror" value="{{ $outlet->latitude }}" type="text" name="latitude"
            ...
            <div>
                ...
                <select id="city" name="city" class=" w-full">
                    ...
                    @foreach ($cities as $city)
                        <option value="{{$city->id}}" {{$city->id == $outlet->city ? 'selected' : ''}}>{{$city->name}}</option>
                    @endforeach
                </select>
                @if ($errors->has('city'))
       ...
       <div class="flex justify-between items-center w-full">
           ...
           <div class="relative inline-flex items-center">
               <input type="checkbox" 
                      ...
                      id="gih-checkbox" 
                      class="sr-only"
                      {{ $form->answers->where('outlet_id', $outlet->id)->first()?->answer == 'true' ? 'checked' : '' }} />
